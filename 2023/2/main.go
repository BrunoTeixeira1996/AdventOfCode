...
func parseInput(input string) []Game {
	...
	for scanner.Scan() {
		...
		bags := []Bag{}
		game.ID = gameCounter
		sets := strings.Split(strings.Split(scanner.Text(), ":")[len(strings.Split(scanner.Text(), ":"))-1], ";")
		for _, v := range sets {
			bag := Bag{}
			temp := strings.Split(v, ",")
			for _, x := range temp {
				if strings.Contains(x, "blue") {
					bag.Blue, _ = strconv.Atoi(strings.Split(strings.TrimSpace(x), " ")[0])
					gotBlue = true
				} else if strings.Contains(x, "red") {
					bag.Red, _ = strconv.Atoi(strings.Split(strings.TrimSpace(x), " ")[0])
					gotRed = true
				} else if strings.Contains(x, "green") {
					bag.Green, _ = strconv.Atoi(strings.Split(strings.TrimSpace(x), " ")[0])
					gotGreen = true
				}
			}
			if !gotBlue {
...
func firstPart(games []Game) {
	...
	for _ , g := range games {
		...
		for _ , bag := range g.Bags {
			if bag.Blue <= 14 && bag.Red <= 12 && bag.Green <= 13 {
				count += 1
			}
		}
		if bagLen == count {
