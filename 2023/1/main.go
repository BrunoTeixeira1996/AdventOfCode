...
const inputFirst = `76xkqjzqtwonfour
...
// FIRST CHALLENGE
func getFirstInt(input string) []int {
	var first []int
	scanner := bufio.NewScanner(strings.NewReader(input))
	for scanner.Scan() {
		for _, v := range scanner.Text() {
			if unicode.IsDigit(v) {
				firstInt, _ := strconv.Atoi(string(v))
...
func getLastInt(input string) []int {
	var last []int
	var lastInt int
	scanner := bufio.NewScanner(strings.NewReader(input))
	for scanner.Scan() {
		for _, v := range scanner.Text() {
			if unicode.IsDigit(v) {
				lastInt, _ = strconv.Atoi(string(v))
			}
		}
		last = append(last, lastInt)
	}
	return last
}

func firstSolution(firstInts []int, secondInts []int) int {
	var result int
	for i, v := range firstInts {
		group := fmt.Sprintf("%d%d", v, secondInts[i])
		number, _ := strconv.Atoi(group)
...
const inputSecond = `76xkqjzqtwonfour
sixthree8sixjxjqsjgjgp
38bgcczgtninefivefive
sixthree4eight
nhp3zdc
...
func checkPartialInSlice(pointer int, partial string) bool {
	for _, v := range alphabetNumbers {
...
func secondSolution(input string) int {
	...
	for scanner.Scan() {
		...
		for i, v := range scanner.Text() {
			...
			// check forward
			for pointer := 0; i <= lenWord ; pointer += 1 {
				if pointer + i == lenWord {
					break
				}

				checkWord += string(scanner.Text()[i+pointer])
				val, ok := numbers[checkWord]
				if ok {
					checkWord = ""
					word = ""
					...
					if checkPartialInSlice(pointer, checkWord) {
						word = checkWord
						fmt.Println(word)
					} else {
						checkWord = ""
		...
		word = ""
		checkWord = ""
		lFirsts = append(lFirsts, first)
		first = nil
	}

	for _ , v := range lFirsts {
			group := fmt.Sprintf("%d%d", v[0], v[len(v)-1])
			number, _ := strconv.Atoi(group)
			result = result + number
			group = ""
	}
...
func main() {
	//fmt.Println(firstSolution(getFirstInt(testInput), getLastInt(testInput)))
	//fmt.Println(firstSolution(getFirstInt(inputFirst), getLastInt(inputFirst)))
	//fmt.Println(secondSolution(testInputSecond))
	fmt.Println(secondSolution(inputSecond))
}
